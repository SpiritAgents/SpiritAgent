...
[Application Desktop](#desktop) · [CLI](#cli) · [ACP Server](#acp-server) · [Agent Core](#agent-core) · [Développement](#développement)
...
- **Modes Agent / Plan / Ask** — accès complet aux outils, workflows de planification uniquement, ou Q&A en lecture seule sans outils d’édition au niveau contrat.
...
- **Mode prompts** — frontières Agent, Plan et Ask sans relister les outils dans le texte system.
...
```bash
npm run dev:cli    # build des packages TS, puis cargo run -p spirit-agent
```
...
**Prérequis :** Node.js 22+, npm. Toolchain Rust requise pour la CLI.
...
| `npm run build` | Build production agent-core, host-internal, acp-server et Desktop |
