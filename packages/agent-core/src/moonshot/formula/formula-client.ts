...
function readFiberError(fiber: FormulaFiberResponse): string | undefined {
  ...
  if (typeof context.output === "string" && context.output.trim() && fiber.status !== "succeeded") {
...
function readFiberSucceededContent(fiber: FormulaFiberResponse): string | undefined {
  ...
  const encrypted =
    typeof context.encrypted_output === "string" ? context.encrypted_output : undefined;
...
export async function invokeFormulaFiber(
  ...
  const error =
    readFiberError(fiber) ?? `Moonshot Formula fiber failed (status=${status || "unknown"}).`;
