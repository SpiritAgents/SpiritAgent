...
function googleStreamChunks(text: string): string[] {
  return [
    `data: ${JSON.stringify({
      candidates: [{
        content: {
...
async function main(): Promise<void> {
  ...
  const server = createServer(async (request, response) => {
    ...
    for (const chunk of googleStreamChunks('AI_SDK_GOOGLE_OK')) {
  ...
  printSmokeSection('ai-sdk google streaming smoke events', events);
  printSmokeSection('ai-sdk google streaming smoke completion', completion);
  ...
  if (assistantText !== 'AI_SDK_GOOGLE_OK') {
    throw new Error(`ai-sdk google streaming smoke 未拿到预期最终 assistant 文本。实际: ${assistantText ?? '<empty>'}`);
  }
...
main().catch((error: unknown) => {
  ...
  console.error(`ai-sdk google streaming smoke failed: ${message}`);
  process.exitCode = 1;
});
