...
async function main(): Promise<void> {
  const server = createServer(async (request, response) => {
    ...
    const hasApplyPatchTool = tools.some(
  ...
  if (firstRound.kind !== "success" || firstRound.result.step.kind !== "tool-calls") {
    throw new Error("ai-sdk gateway apply_patch smoke did not reach tool-calls.");
  }

  const call = firstRound.result.step.calls[0];
  if (!call || call.name !== "apply_patch") {
    throw new Error("ai-sdk gateway apply_patch smoke did not receive an apply_patch tool call.");
  }
}
