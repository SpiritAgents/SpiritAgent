...
test("isOpenRouterAnthropicClaudeModel matches openrouter anthropic routes only", () => {
  assert.equal(isOpenRouterAnthropicClaudeModel("openrouter", "anthropic/claude-sonnet-4.6"), true);
  assert.equal(
    isOpenRouterAnthropicClaudeModel("vercel-ai-gateway", "anthropic/claude-sonnet-4.6"),
    false,
  );
  assert.equal(isOpenRouterAnthropicClaudeModel("openrouter", "openai/gpt-5"), false);
});
