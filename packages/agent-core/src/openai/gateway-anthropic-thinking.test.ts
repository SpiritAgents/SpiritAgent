...
test("isGatewayAnthropicClaudeModel matches vercel-ai-gateway anthropic routes only", () => {
  ...
  assert.equal(isGatewayAnthropicClaudeModel("vercel-ai-gateway", "openai/gpt-5"), false);
  assert.equal(isGatewayAnthropicClaudeModel("openrouter", "anthropic/claude-sonnet-4.6"), false);
});
...
test("gatewayAnthropicClaudeSupportedEfforts exports effort vocabulary for catalog metadata", () => {
  assert.deepEqual(gatewayAnthropicClaudeSupportedEfforts("anthropic/claude-sonnet-4.6"), [
    "low",
    "medium",
    "high",
  ]);
