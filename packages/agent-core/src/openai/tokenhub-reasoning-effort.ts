...
export function isTokenHubReasoningEffortModel(context?: ModelReasoningEffortContext): boolean {
  if (context?.provider !== "tencent-tokenhub") {
    return false;
  }
  return TOKEN_HUB_REASONING_EFFORT_MODEL_IDS.has(normalizeUpstreamModelId(context.model ?? ""));
}
