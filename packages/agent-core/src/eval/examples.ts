...
export function createExampleEvalScenario(): EvalScenario {
  return {
    ...
    description:
      "A human-reviewable scenario for comparing prompt/runtime changes on frontend implementation quality.",
    userPrompt: "帮我写一个视觉完成度很高、可直接运行的产品网站，主题是个人知识库工具。",
    workspaceBrief:
      "Host creates an isolated frontend workspace for each variant and runs the same setup steps.",
...
export function createExampleEvalRunSpec(): EvalRunSpec {
  ...
  return {
    ...
    isolation: {
      ...
      description:
        "agent-core declares the contract only; host-internal/apps provide isolated execution.",
    },
    artifactPolicy: {
