...
export const evalScenarios: EvalScenario[] = [
  {
    ...
    description:
      "Compare whether baseline and candidate can plan, create files, run commands, inspect output, and deliver a usable result.",
    userPrompt:
      "在当前空工作区里实现一个可运行的小型前端页面：主题是个人知识库工具。要求包含清晰的信息架构、至少一个交互控件、基础响应式布局，并在完成后说明如何运行或打开。",
    workspaceBrief:
      "The runner creates an isolated empty workspace for each variant unless --workspace-source is provided.",
    ...
    rubric: {
      ...
      criteria: [
        ...
        },
        {
          ...
          label: "Tool judgment",
          description:
            "Tool use is necessary, scoped, sequenced well, and visible in the run trace.",
          weight: 0.25,
          scale: { min: 1, max: 5 },
        },
        {
          ...
          description:
            "System, repository, and user instructions are followed without drifting into unrelated work.",
          weight: 0.2,
          scale: { min: 1, max: 5 },
        },
        {
          ...
          label: "Reviewability",
          description:
            "The human reviewer can easily inspect workspace changes, artifacts, and final output.",
          weight: 0.2,
          scale: { min: 1, max: 5 },
        },
      ],
    ...
    metadata: {
      ...
      decisionPrompt:
        "Compare baseline and candidate outputs for this scenario. Pick the better user experience, or tie if neither is meaningfully better.",
    },
  },
  {
    ...
    description:
      "A frontend-heavy scenario for human review of visual quality and implementation ergonomics.",
    userPrompt:
      "帮我写一个视觉完成度很高、可直接运行的产品网站，主题是个人知识库工具。需要第一屏足够完整，移动端也要可用。",
    workspaceBrief:
      "The runner creates an isolated empty workspace for each variant unless --workspace-source is provided.",
    expectedDeliverables: [
      ...
      criteria: [
        {
          ...
          description:
            "Layout, hierarchy, typography, spacing, color, and domain fit feel polished.",
          weight: 0.4,
          scale: { min: 1, max: 5 },
        },
        {
    ...
    metadata: {
      ...
      decisionPrompt:
        "Compare baseline and candidate workspaces for visual quality, usability, and reviewability.",
    },
  },
];
