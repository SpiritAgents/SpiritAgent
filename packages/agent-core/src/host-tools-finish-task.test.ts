...
test("buildBuiltinHostToolDefinitions omits finish_task", () => {
  const names = buildBuiltinHostToolDefinitions(environment).map((definition) =>
    readToolName(definition),
  );
...
test("assertFinishTaskToolAllowed rejects finish_task when Loop is off", () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof Error && error.message === unknownToolErrorMessage("finish_task", available),
  );
