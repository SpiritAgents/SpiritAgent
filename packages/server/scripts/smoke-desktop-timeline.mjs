...
async function main() {
  ...
  await a.call('server.initialize', { clientKind: 'desktop', clientId: 'smoke-timeline-a', workspaceRoot: WORKSPACE });
  ...
  await b.call('server.initialize', { clientKind: 'cli', clientId: 'smoke-timeline-b', workspaceRoot: WORKSPACE });
  ...
  assertDeepEqual(archive.desktopMessageTimeline, TIMELINE_V1, 'exportArchive desktopMessageTimeline');
  if (archive.desktopMessageTimelineRevision !== 1) {
    throw new Error(`exportArchive revision: expected 1, got ${archive.desktopMessageTimelineRevision}`);
  }
  ...
  if (pulled.revision !== 1) throw new Error(`getDesktopTimeline revision: expected 1, got ${pulled.revision}`);
  ...
  const notified = new Promise((resolve, reject) => {
    ...
    b.onNotification((msg) => {
      if (msg.method === 'session.desktopTimelineUpdated' && msg.params?.sessionId === session.sessionId) {
        clearTimeout(timer);
  ...
  if (notifiedRevision !== 2) throw new Error(`notification revision: expected 2, got ${notifiedRevision}`);
  ...
  let rejected = false;
  try {
    await a.call('session.pushDesktopTimeline', { sessionId: session.sessionId, timeline: { nope: true } });
  } catch (error) {
