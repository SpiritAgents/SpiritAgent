/**
 * Applies headerless V4A diffs (OpenAI apply_patch format).
...
export function applyDiff(input: string, diff: string, mode: ApplyDiffMode = 'default'): string {
  ...
  return applyChunks(input, chunks, newline);
}
...
function parseUpdateDiff(lines: string[], input: string): { chunks: Chunk[]; fuzz: number } {
  ...
  while (!isDone(parser, END_SECTION_MARKERS)) {
    ...
    cursor = findResult.newIndex + section.nextContext.length;
    parser.fuzz += findResult.fuzz;
    parser.index = section.endIndex;

    for (const ch of section.sectionChunks) {
