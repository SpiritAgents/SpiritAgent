...
import {
  ...
  GOOGLE_GEMINI_OPENAI_COMPAT_BASE,
...
test('isGoogleGeminiGenerativeLanguageApiBase accepts Gemini preset and native roots', () => {
  assert.equal(isGoogleGeminiGenerativeLanguageApiBase(GOOGLE_GEMINI_OPENAI_COMPAT_BASE), true);
  assert.equal(isGoogleGeminiGenerativeLanguageApiBase(`${GOOGLE_GEMINI_OPENAI_COMPAT_BASE}/`), true);
...
test('googleNativeModelsListUrl derives native models endpoint from OpenAI compat base', () => {
  assert.equal(
    googleNativeModelsListUrl(GOOGLE_GEMINI_OPENAI_COMPAT_BASE),
    `${GOOGLE_GEMINI_NATIVE_API_ROOT}/models?pageSize=1000`,
  );
  assert.equal(
    googleNativeModelsListUrl(GOOGLE_GEMINI_OPENAI_COMPAT_BASE, 'page-2'),
