---
title: クイックスタート
description: Spirit Agent をインストールし、プロバイダを1つ接続して、小さな承認済みの編集を行います。
---
...
すでに使用しているプロバイダの API キーが必要です。Spirit はそのキーをプロジェクトではなく、OS のキーリング（`SpiritAgent`）に保存します。
...
<Callout title="キーリング">
キーは、Spirit が `config.json` を更新する前にシステムキーリングに書き込まれます。後のページで再接続を求められた場合、リポジトリにキーを貼り付けるのではなく、キーリングエントリを復元します。
</Callout>
