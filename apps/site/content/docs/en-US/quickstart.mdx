---
title: Quickstart
description: Install Spirit Agent, connect one provider, and land a small approved edit.
---
...
You need an API key from a provider you already use. Spirit stores that key in the operating system keyring (`SpiritAgent`), not in the project.
...
The key is written to the system keyring before Spirit updates `config.json`. If a later page asks you to reconnect, you are restoring the keyring entry, not pasting a key into the repo.
</Callout>
...
- [Providers](./providers/index.mdx) to connect another house, or to check site / plan / cloud fields when models did not import
