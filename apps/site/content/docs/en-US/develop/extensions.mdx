---
title: Extensions
description: Package layout, spiritExtension fields, activate API, and listing on the official registry.
---
...
`supportedHosts` must include the current host. A second install of the same id does not replace the existing copy by default. See [Marketplace](../customize/marketplace.mdx) for channels and the install UI.

## Publish to the official registry

The [SpiritAgents/registry](https://github.com/SpiritAgents/registry) repository is a marketplace index. It does not host extension source, `dist`, ZIP files, or tarballs.
...
Official example: [`@spiritagent/extension-system-message-demo`](https://www.npmjs.com/package/@spiritagent/extension-system-message-demo) (`spirit.system-message-demo`).
...
`extensionId` needs at least two dot-separated segments, lowercase letters and digits, with dots or hyphens inside a segment. Examples: `spirit.system-message-demo`, `yourteam.some-extension`. `packageName` and `extensionId` must be unique in the repository.
