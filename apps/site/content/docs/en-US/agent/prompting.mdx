...
On Desktop, attach open files, selections, or a Browser element when the task is about that exact spot. The agent still has to read the file to see the current contents — attachments are pointers, not a substitute for a fresh read.
