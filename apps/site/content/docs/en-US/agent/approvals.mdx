---
title: Approvals
description: Default, auto-approval, and full-approval (Bypass). What each level still stops.
---
...
| Bypass Approval | `full-approval` | High-risk tools run without the approval prompt. |

## What bypass does not skip

`full-approval` still stops for:
...
Headless `spirit -p` fails if a call needs approval. Pass `-a auto-approval` or `-a full-approval`, or use the TUI.
