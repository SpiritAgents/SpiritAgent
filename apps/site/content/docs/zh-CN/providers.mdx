...
- **区域站点** — Moonshot AI、SiliconFlow、MiniMax（中国 / 国际）；Alibaba（区域 + `workspaceId`）；腾讯 TokenHub（广州 / 新加坡）
- **套餐端点** — Alibaba Token Plan、StepFun Step Plan、Z.ai / 智谱 AI GLM Coding Plan
...
在 [Xiaomi MiMo 控制台](https://platform.xiaomimimo.com/console/api-keys) 创建 API Key。
...
在 [StepFun 平台](https://platform.stepfun.com) 创建 API Key。套餐说明见 [Step Plan](https://platform.stepfun.com/docs/zh/step-plan/integrations/reasoning-api)。
