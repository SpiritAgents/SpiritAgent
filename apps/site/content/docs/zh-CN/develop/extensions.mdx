---
title: 扩展
description: 包结构、spiritExtension 字段、activate API，以及上架官方 registry。
---
...
官方示例：[`@spiritagent/extension-system-message-demo`](https://www.npmjs.com/package/@spiritagent/extension-system-message-demo)（`spirit.system-message-demo`）。
...
`extensionId` 至少两段、用点分隔，小写字母和数字，段内可用点或连字符。例如：`spirit.system-message-demo`、`yourteam.some-extension`。`packageName` 与 `extensionId` 在仓库内都必须唯一。
