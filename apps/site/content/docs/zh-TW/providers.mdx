---
title: 供應商
description: 自備金鑰、執行連線精靈，切換供應商時也保持相同的代理工作流程。
---

Spirit Agent 是 BYOK。你在供應商控制台建立金鑰，將其貼到 Desktop 或 CLI，Spirit 會將它存放在作業系統的金鑰環中。切換供應商不會改變你開啟工作區、批准編輯或執行代理的方式。

連線精靈在 Desktop 是 **Settings → Models**，在 CLI 是 `/model add` 或 `spirit model add`。只有當供應商需要時才會出現額外欄位。
...
- **區域站點** — Moonshot AI、SiliconFlow、MiniMax（中國 / 國際）；阿里雲（區域 + `workspaceId`）；騰訊 TokenHub（廣州 / 新加坡）
- **方案端點** — 阿里雲 Token 方案、StepFun Step 方案、Z.ai / 智譜 AI GLM 編碼方案
...
在 [Tencent TokenHub 控制台](https://console.cloud.tencent.com/tokenhub) 建立 API 金鑰。
