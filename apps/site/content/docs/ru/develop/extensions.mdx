---
title: Расширения
description: Структура пакета, поля spiritExtension, API активации и список в официальном реестре.
---
...
Официальный пример: [`@spiritagent/extension-system-message-demo`](https://www.npmjs.com/package/@spiritagent/extension-system-message-demo) (`spirit.system-message-demo`).
...
`extensionId` должен содержать как минимум два сегмента, разделенных точкой, состоящих из строчных букв и цифр, с точками или дефисами внутри сегмента. Примеры: `spirit.system-message-demo`, `yourteam.some-extension`. `packageName` и `extensionId` должны быть уникальными в репозитории.
