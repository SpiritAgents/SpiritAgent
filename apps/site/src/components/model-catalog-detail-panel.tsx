...
export function ModelCatalogDetailPanel({
  ...
  return (
    <div className={cn(isList ? "flex flex-col" : "-mx-3 flex flex-col")}>
      ...
      >
        <p
          className={cn(
            isList
              ? DESKTOP_OVERLAY_LIST_ITEM_PRIMARY
              : `text-sm ${FONT_WEIGHT_NORMAL} leading-snug text-foreground`,
          )}
        >
        ...
        >
          <NoTranslate>{providerLabel}</NoTranslate>
        </p>
        {capabilities && capabilities.length > 0 ? (
          <div className={cn("flex flex-wrap gap-1.5", isList ? "mt-1.5" : "mt-2")}>
            {capabilities.map((capability: DesktopModelCapability) => (
              <Badge
                key={capability}
                variant="secondary"
      ...
      {hasBody ? (
        <div
          className={cn(isList ? "space-y-2 px-2 py-1.5 text-xs" : "space-y-3 px-3 pt-3 text-xs")}
        >
          {description ? (
