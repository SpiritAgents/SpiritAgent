...
pub(crate) fn help_text(input_mode: MainInputMode) -> String {
    ...
    lines.extend([
        ...
        "- /rules".to_string(),
        "- /create-skill [repo|user] <skill-name> <需求描述>".to_string(),
        "- /skills".to_string(),
        ...
        "- /create-skill 会走正常 assistant 对话来起草或收紧 SKILL.md；repo 目标默认写入工作区 .spirit/skills/<skill-name>/SKILL.md，user 目标写入 Spirit 用户目录 skills/<skill-name>/SKILL.md，都会走标准工具审批。".to_string(),
