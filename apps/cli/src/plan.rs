...
mod tests {
    ...
    #[test]
    fn build_create_plan_user_turn_mentions_plan_write_scope_and_exact_reminder() {
        let workspace_root = PathBuf::from("C:/workspace/demo");
        let prompt = build_create_plan_user_turn(&workspace_root, "实现 Spirit Plan 模式");

        assert!(prompt.contains("正常流式输出"));
        assert!(prompt.contains("给后续实现阶段的 LLM 和宿主看的执行文档"));
        assert!(prompt.contains("Spirit 托管的用户目录"));
        assert!(prompt.contains("create_file 或 update_file"));
        assert!(prompt.contains("plan.md"));
        assert!(prompt.contains(START_IMPLEMENTING_REMINDER));
    }
