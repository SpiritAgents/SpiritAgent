...
fn process_key_event(
    ...
    if shell.is_access_picker_active() {
        match key.code {
            KeyCode::Esc => shell.cancel_access_picker(),
            KeyCode::Up => shell.select_prev_access_level(),
            KeyCode::Down => shell.select_next_access_level(),
            KeyCode::Enter => shell.confirm_access_picker(),
            KeyCode::Char('c') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                shell.request_quit();
            }
            _ => {}
