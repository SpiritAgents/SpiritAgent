...
export async function generateSessionTitleFromModelTask(
  ...
  const result = await transport.createJsonSchemaCompletion<{ title: string }>(transportConfig, {
    userPrompt,
    schemaName: "session_title",
    schema: SESSION_TITLE_JSON_SCHEMA,
    includeToolAgentHostPrompt: false,
    systemSections: [
      buildSpiritAgentCoreHostPrompt(resolved.name, resolved.profile.provider),
    ],
  });
