...
import { mergeAdjacentTextSegments } from "./composer-segment-model";
...
export function removeInlineChipAtRemovalPoint(
  ...
  const next = mergeAdjacentTextSegments([
    ...merged.slice(0, chipIndex),
    ...merged.slice(chipIndex + 1),
  ]);
  const caretSegmentIndex = Math.min(chipIndex, Math.max(next.length - 1, 0));
  const at = next[caretSegmentIndex];
  ...
  return { segments: next, caret: nextCaret };
}
