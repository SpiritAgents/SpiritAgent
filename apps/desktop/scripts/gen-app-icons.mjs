...
function verifyPackagerIcons() {
  ...
  if (missing.length > 0) {
    const list = missing
      .map(({ label, path: filePath }) => `- ${label}: ${path.relative(desktopRoot, filePath)}`)
      .join("\n");
    throw new Error(
...
async function flattenPackagerIcons() {
  ...
  console.log(
    `Synced ${path.relative(desktopRoot, iconIcoPath)} and ${path.relative(desktopRoot, faviconIcoPath)}`,
  );
  await assertOpaqueBlackCanvas(iconPngPath);
}
