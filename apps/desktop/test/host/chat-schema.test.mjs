...
test("normalizeTimelineSnapshotForPersistence omits empty content for tool and thinking rows", () => {
  const normalized = normalizeTimelineSnapshotForPersistence(
    sampleTurn([
...
test("validateTimelineSnapshotV2 rejects empty assistant-text and legacy content on tool rows", () => {
  const valid = normalizeTimelineSnapshotForPersistence(
    sampleTurn([thinkingRow("reasoning"), toolRow("call-1"), textRow("answer")]),
  );
...
test("assertChatSchemaVersionV2 and assertNoLegacyConversationFields enforce v2 shape", () => {
  ...
  assert.throws(
    () => assertNoLegacyConversationFields({ assistantAux: [] }),
    ChatSessionSchemaError,
  );
  assert.throws(
    () => assertNoLegacyConversationFields({ desktopMessages: [] }),
    ChatSessionSchemaError,
  );
