...
test("formatProcessGroupSummary falls back to thought count when no tools", () => {
  ...
  assert.equal(formatProcessGroupSummary(t, emptyProcessToolCounts(), messages, [1]), "1 Thought");
  assert.equal(
...
test("formatProcessGroupSummary orders categories by tool chronology", async () => {
  ...
  const askThenCreateTools = askThenCreateMessages.map((message) => message.tool).filter(Boolean);
  assert.equal(
    formatProcessGroupSummary(
      tZh,
  ...
  const createThenAskTools = createThenAskMessages.map((message) => message.tool).filter(Boolean);
  assert.equal(
    formatProcessGroupSummary(
      tZh,
  ...
  const repeatedAskTools = repeatedAskMessages.map((message) => message.tool).filter(Boolean);
  assert.equal(
    formatProcessGroupSummary(
      tZh,
